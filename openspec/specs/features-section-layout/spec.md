...
Define el comportamiento de la retícula de tarjetas de la sección "¿Por qué elegirnos?": cuántas columnas se muestran en cada breakpoint, cómo se dimensiona verticalmente cada tarjeta y qué garantías existen sobre la ausencia de desbordamiento en cualquier ancho de viewport.

El punto frágil de esta sección es el acoplamiento entre alto y ancho: el copy de las features vive en `AppConstants.features` y está diseñado para cambiar, así que la altura de la tarjeta MUST derivarse del contenido y nunca de un ratio fijo respecto del ancho de la celda. Los breakpoints (600 px y 1024 px) siguen el mismo patrón inline de `MediaQuery` que el resto de las secciones del landing.
